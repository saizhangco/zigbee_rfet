...
#define GUID_STATE     0x02
#define ADDR_STATE     0x03
#define CMD_STATE      0x04
...
#ifdef ZS_DEBUG_UART
    HalUARTWrite(0x00,"+",1);
    HalUARTWrite(0x00,&ch ,1);
    HalUARTWrite(0x00," " ,1);
    ch2 = ch / 16;
    ch2 = ch2 < 10 ? ch2+'0':ch2+'A'-10;
    HalUARTWrite(0x00,&ch2 ,1);
    ch2 = ch % 16;
    ch2 = ch2 < 10 ? ch2+'0':ch2+'A'-10;
    HalUARTWrite(0x00,&ch2 ,1);
    HalUARTWrite(0x00,"\n",1);
    ...
    switch(state)
    {
        ...
        case LEN_STATE:
            ...
            pMsg = (mtOSALSerialData_t *)osal_msg_allocate( sizeof ( mtOSALSerialData_t ) +
                                                        SRF_FRAME_HDR_SZ + LEN_Token );
            if (pMsg)
            {
              /* Fill up what we can */
              pMsg->hdr.event = CMD_SERIAL_MSG;
              pMsg->msg = (uint8*)(pMsg+1);
              pMsg->msg[SRF_POS_LEN] = LEN_Token;
              state = GUID_STATE;
              POSI_Token = 0;
#ifdef ZS_DEBUG_UART
              HalUARTWrite(0,"GUID_STATE\n" ,11);  /* 调试使用 */
#endif
            }
            else
            {
                state = SOP_STATE;
#ifdef ZS_DEBUG_UART
                HalUARTWrite(0,"SOP_STATE\n" ,10);  /* 调试使用 */
#endif
                return;
            }
            break;
        case GUID_STATE:
            pMsg->msg[SRF_POS_ID0 + POSI_Token++] = ch;
            if( POSI_Token >= 4 )
            {
                state = ADDR_STATE;
                POSI_Token = 0;
#ifdef ZS_DEBUG_UART
                HalUARTWrite(0,"ADDR_STATE\n" ,11);  /* 调试使用 */
#endif
            }
            break;
        case ADDR_STATE:
            pMsg->msg[SRF_POS_SA0 + POSI_Token++] = ch;
            if( POSI_Token >= 4 )
            {
                state = CMD_STATE;
                POSI_Token = 0;
#ifdef ZS_DEBUG_UART
                HalUARTWrite(0,"CMD_STATE\n" ,10);  /* 调试使用 */
#endif
            }
            break;
        case CMD_STATE:
            pMsg->msg[SRF_POS_CMD0 + POSI_Token++] = ch;
            if( POSI_Token >= 4 )
            {
                if( LEN_Token )
                {
        ...
        case DATA_STATE:
            ...
            pMsg->msg[SRF_FRAME_HDR_SZ + tempDataLen++] = ch;
            ...
            {
                HalUARTRead (port, &pMsg->msg[SRF_FRAME_HDR_SZ + tempDataLen], bytesInRxBuffer);
                tempDataLen += bytesInRxBuffer;
            }
            else
            {
                HalUARTRead (port, &pMsg->msg[SRF_FRAME_HDR_SZ + tempDataLen], LEN_Token - tempDataLen);
...
//======================================
    ...
    switch (state)
    {
      ...
      case DATA_STATE:
        ...
        /* If number of bytes read is equal to data length, time to move on to FCS */
#ifdef ZS_DEBUG_UART
        ch1 = bytesInRxBuffer + '0';
        HalUARTWrite(0,&ch1 ,1);  /* 调试使用 */
        ch1 = tempDataLen + '0';
        HalUARTWrite(0,&ch1 ,1);  /* 调试使用 */
        ch1 = LEN_Token + '0';
        HalUARTWrite(0,&ch1 ,1);  /* 调试使用 */
        ...
        HalUARTWrite(0x00,&pMsg->msg[MT_RPC_POS_DAT0] ,pMsg->msg[MT_RPC_POS_LEN]);  /* 调试使用 */
        HalUARTWrite(0x00,"]\n" ,2);  /* 调试使用 */
#endif
        
        //if ( tempDataLen == LEN_Token-'0' )
            //state = FCS_STATE;
        osal_msg_send( App_TaskID, (uint8 *)pMsg );
